...
public class ShopAccounts extends javax.swing.JPanel {
   ...
   private static PrintService findPrintService(String printerName) {
    ...
    private void initComponents() {
        ...
        jLabel23.setText("Cost Stock Returned to Supplier");
        ...
        try {
            ...
            String sql1="SELECT Return_Amount FROM ReturnToSupplier";
            ...
            String amm=String.valueOf(am);
            jTextField15.setText(amm);

            Statement stt=connection.createStatement();
            ResultSet rs15=stt.executeQuery("SELECT TotalReturnedAmount, oneTimeid FROM Customers_Returned_Products ORDER BY oneTimeid ASC");

            int treturned=0;
            int preinvoice=0;
            int curinvoice=0;
            while(rs15.next()){
                curinvoice=rs15.getInt(2);

                if(curinvoice!=preinvoice){
                    treturned+=(int)rs15.getDouble(1);
                }
                preinvoice=curinvoice;
            }

            String s=String.valueOf(treturned);
        ...
        jLabel24.setText("Cost Stock Returned By Customer");

        jLabel11.setFont(new java.awt.Font("Segoe UI", 3, 18)); // NOI18N
        jLabel11.setText("Return Stock Account");
...
ResultSet rs15=stt.executeQuery("SELECT Datetime, cost, sales, Discount, Actual_bill FROM Sales"); 
    ...
    int pay=0;
        ...
        while(rs15.next()){
                                if((((rs15.getDate(1).toLocalDate()).isAfter( date1))||((rs15.getDate(1).toLocalDate()).isEqual(date1))) && ((rs15.getDate(1).toLocalDate().isBefore(date))||(rs15.getDate(1).toLocalDate().isEqual(date)))){
...
ResultSet rs16=stt.executeQuery("SELECT E_Name, Date_Time, TotalBill FROM Expenses"); 
        ...
        while(rs16.next()){
                                            ...
                                            if((((rs16.getDate(2).toLocalDate()).isAfter( date1))||((rs16.getDate(2).toLocalDate()).isEqual(date1))) && ((rs16.getDate(2).toLocalDate().isBefore(date))||(rs16.getDate(2).toLocalDate().isEqual(date)))){
...
ResultSet rs17=stt.executeQuery("SELECT P_Cost, Added_from,P_Quantity, StockAddedDate FROM StockAdded"); 
        ...
        while(rs17.next()){
            if((((rs17.getDate(4).toLocalDate()).isAfter( date1))||((rs17.getDate(4).toLocalDate()).isEqual(date1))) && (rs17.getDate(4).toLocalDate().isBefore(date))||(rs17.getDate(4).toLocalDate().isEqual(date))){
...
int tcost=(newcost+rtcost);
    ...
    Statement st= connection.createStatement();
    String sql1="SELECT Return_Amount, DateandTime FROM ReturnToSupplier";
    ResultSet rs =st.executeQuery(sql1);
              
    int am=0;
    while(rs.next()){
        
                     if((((rs.getDate(2).toLocalDate()).isAfter( date1))||((rs.getDate(2).toLocalDate()).isEqual(date1))) && (rs.getDate(2).toLocalDate().isBefore(date))||(rs.getDate(2).toLocalDate().isEqual(date))){
...
String amm=String.valueOf(am);
    ...
    Statement sttw=connection.createStatement();
ResultSet rs15e=sttw.executeQuery("SELECT TotalReturnedAmount, oneTimeid, DateTime FROM Customers_Returned_Products ORDER BY oneTimeid ASC"); 
...
int curinvoice=0;
        while(rs15e.next()){
             if((((rs15e.getDate(3).toLocalDate()).isAfter( date1))||((rs15e.getDate(3).toLocalDate()).isEqual(date1))) && (rs15e.getDate(3).toLocalDate().isBefore(date))||(rs15e.getDate(3).toLocalDate().isEqual(date))){
            
            
curinvoice=rs15e.getInt(2);

if(curinvoice!=preinvoice){
treturned+=(int)rs15e.getDouble(1);
} 
       preinvoice=curinvoice;   
